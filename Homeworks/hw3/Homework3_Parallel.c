...
void *pushAndPop(void *data) {
  ...
  while (isSuspended) {
    // mutex unlocked, thread goes to sleep
  ...
  for (i = 0; i < end; i++) {
    Pop(); // comment if outputting popped values
...
int main(int argc, char *argv[]) {
  ...
  // timing_start(); // Moved to remove thread creation from timing calculation and try cond var
